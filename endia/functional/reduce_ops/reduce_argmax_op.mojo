...
struct ReduceArgMax(DifferentiableReduceOp):
    @staticmethod
    ...
    fn __call__(inout curr: Array, args: List[Array]) raises:
        ...
        var arg_max_stride = target_stride
        var denom = 1
        for i in range(rank):
        ...
        fill_(curr, -1)
        var tmp_max = full(target_shape, min_or_neg_inf[dtype]())

        var target_storage_offset = curr.storage_offset()
        var tmp_max_data = tmp_max.data()
        ...
        if rank != 1:
            ...
            for i in range(0, arg.size(), rows * cols):
                ...
                var arg_max_idx_0 = compute_storage_offset(
                    nd_idx, arg_max_stride, target_storage_offset
                )
                for j in range(rows):
                    ...
                    var arg_max_idx_1 = arg_max_idx_0 + j * arg_max_stride[
                        rank - 2
                    ]
                    ...
                    for k in range(cols):
                        ...
                        var arg_max_idx = arg_max_idx_1 + k * arg_max_stride[
                            rank - 1
                        ]

                        var new_curr = max(
                            tmp_max_data.load(target_idx),
                            arg_data.load(base_idx),
                        )
                        if new_curr > tmp_max_data.load(target_idx):
                            curr_data.store(target_idx, arg_max_idx // denom)

                        tmp_max_data.store(target_idx, new_curr)
        else:
            ...
            if target_stride[0] == 0:
                ...
                for i in range(0, arg.size()):
                    var new_curr = max(tmp_max_data.load(0), arg_data.load(i))
                    if new_curr != tmp_max_data.load(0):
                        curr_data.store(0, i)
                    tmp_max_data.store(0, new_curr)
            # otherwise, if we we have rank one but not´reduction, we simply copy the values
                ...
                for i in range(end, arg.size()):
                    tmp_max_data.store(i, arg_data.load(i))
    ...
    fn fwd(arg0: Array, axis: List[Int]) raises -> Array:
        ...
        """
        if arg0.is_complex():
            raise "Warning in reduce_argmax:  The reduce_argmax operation does not support complex numbers."

        var arr_shape = setup_array_shape(
            List(arg0.array_shape(), list_to_array_shape(axis)),
            "reduce_arg_max",
            ReduceArgMax.compute_shape,
        )
...
fn reduce_argmax(
    ...
    """
    if keepdims:
        return ReduceArgMax.fwd(arg0, axis)
    return squeeze(ReduceArgMax.fwd(arg0, axis))
