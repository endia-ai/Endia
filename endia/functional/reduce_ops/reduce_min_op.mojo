...
struct ReduceMin(DifferentiableReduceOp):
    @staticmethod
        ...
        if rank != 1:
            ...
            if target_stride[0] == 0:
                ...
                for i in range(end, arg.size()):
                    curr_data.store(0, min(curr_data.load(0), arg_data.load(i)))
    ...
    fn fwd(arg0: Array, axis: List[Int]) raises -> Array:
        ...
        """
        if arg0.is_complex():
            raise "Warning in reduce_min:  The reduce_min operation does not support complex numbers."

        var arr_shape = setup_array_shape(
            List(arg0.array_shape(), list_to_array_shape(axis)),
            "reduce_min",
            ReduceMin.compute_shape,
        )
...
fn reduce_min(
    ...
    """
    if keepdims:
        return ReduceMin.fwd(arg0, axis)
    return squeeze(ReduceMin.fwd(arg0, axis))
